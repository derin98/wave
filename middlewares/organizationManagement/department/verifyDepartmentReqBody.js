...
validateDepartmentId = async (req, res, next) => {
    ...
    let checkExistingDepartment = await DepartmentDbOperations.checkExistingDepartment(req.params.id);
...
validateDepartmentIds = async (req, res, next) => {
    ...
    let invalidDepartmentIds = await DepartmentDbOperations.returnInvalidDepartmentIds(req.body.ids);
